...
| `brand` | Brand reference object: `domain` (the [brand registry](/dist/docs/3.0.0-rc.1/brand-protocol/brand-json) house domain) and optional `brand_id` (sub-brand within the house) |
...
| `account_scope` | How the seller scoped this account: `operator`, `brand`, `operator_brand`, or `agent`. See [account scope](/dist/docs/3.0.0-rc.1/building/integration/accounts-and-agents#account-scope). |
...
- [sync_accounts](/dist/docs/3.0.0-rc.1/accounts/tasks/sync_accounts) — Sync advertiser accounts with a seller
...
- [Brand protocol](/dist/docs/3.0.0-rc.1/brand-protocol/brand-json) — How vendor agents resolve brand identity from the brand's `domain`
