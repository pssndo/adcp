...
See [Policy Compliance](/dist/docs/3.0.0-rc.1/media-buy/media-buys/policy-compliance) for complete implementation guidance.
...
Buyers can discover the agent's portfolio via [`get_adcp_capabilities`](/dist/docs/3.0.0-rc.1/protocol/get_adcp_capabilities), which returns the publisher domains and primary channels the agent represents. This keeps responses lightweight while maintaining full validation capability.
...
See [Authorization Validation](/dist/docs/3.0.0-rc.1/governance/property/adagents#buyer-agent-validation) for complete requirements.
...
**A:** **Agentic eXecution Engine (AXE)** - the real-time execution layer between orchestrators and decisioning platforms. AXE handles dynamic audience targeting (buyers bringing their own segments), brand suitability enforcement, frequency management, and first-party data activation at impression time. See [AXE documentation](/dist/docs/3.0.0-rc.1/media-buy/advanced-topics/agentic-execution-engine) for complete details.

## Need More Help?

If your question isn't answered here:

1. Check the [Task Reference](/dist/docs/3.0.0-rc.1/media-buy/task-reference/) for detailed API documentation
2. Review [Brief Expectations](/dist/docs/3.0.0-rc.1/media-buy/product-discovery/brief-expectations) for discovery guidance
3. See [Media Products](/dist/docs/3.0.0-rc.1/media-buy/product-discovery/media-products) for product model details
