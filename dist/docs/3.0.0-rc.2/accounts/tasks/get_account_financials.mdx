...
`get_account_financials` is only available when the seller declares `account_financials: true` in [`get_adcp_capabilities`](/dist/docs/3.0.0-rc.2/protocol/get_adcp_capabilities). It applies to **operator-billed accounts** only. For agent-billed accounts, the agent's own billing system is the source of truth.
...
|-----------|------|----------|-------------|
| `account` | object | Yes | [Account reference](/dist/docs/3.0.0-rc.2/building/integration/accounts-and-agents#account-references) — by `account_id` or natural key (`brand` + `operator` + optional `sandbox`). Must be an operator-billed account. |
...
- [list_accounts](/dist/docs/3.0.0-rc.2/accounts/tasks/list_accounts) -- Discover accounts and check their status
- [get_adcp_capabilities](/dist/docs/3.0.0-rc.2/protocol/get_adcp_capabilities) -- Check `account_financials` before calling this task
- [Accounts and agents](/dist/docs/3.0.0-rc.2/building/integration/accounts-and-agents) -- Billing models and account references
