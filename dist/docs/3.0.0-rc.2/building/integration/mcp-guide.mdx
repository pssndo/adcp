...
Transport-specific guide for integrating AdCP using the Model Context Protocol. For task handling, status management, and workflow patterns, see [Task Lifecycle](/dist/docs/3.0.0-rc.2/building/implementation/task-lifecycle).

## Testing AdCP via MCP

You can test AdCP tasks using the [CLI](/dist/docs/3.0.0-rc.2/quickstart) or by chatting with [Addie](https://agenticadvertising.org), the AgenticAdvertising.org assistant.
...
**Task Parameters**: See individual task documentation in [Media Buy](/dist/docs/3.0.0-rc.2/media-buy) and [Signals](/dist/docs/3.0.0-rc.2/signals/overview) sections.

**Task Management**: For comprehensive guidance on tracking async operations, polling patterns, and webhook integration, see [Webhooks](/dist/docs/3.0.0-rc.2/building/implementation/webhooks).
...
See [Webhooks](/dist/docs/3.0.0-rc.2/building/implementation/webhooks) for webhook payload formats and handling examples.
...
<Note>
**Recommended**: Use [`get_adcp_capabilities`](/dist/docs/3.0.0-rc.2/protocol/get_adcp_capabilities) for runtime capability discovery. The server card extension provides static metadata for tool catalogs and registries.
</Note>
...
- Declare which typed extensions you support (see [Context & Sessions](/dist/docs/3.0.0-rc.2/building/integration/context-sessions#extension-fields-ext))
...
- **Protocol Comparison**: Compare with [A2A integration](/dist/docs/3.0.0-rc.2/building/integration/a2a-guide)
- **Examples**: Find complete workflow examples in Core Concepts

**For status handling, async operations, and clarification patterns, see [Task Lifecycle](/dist/docs/3.0.0-rc.2/building/implementation/task-lifecycle) - this guide focuses on MCP transport specifics only.**