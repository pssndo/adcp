...
|--------|----------|----------------|
| **Brand** | Whose products are advertised? | Brand reference: `domain` + optional `brand_id` ([brand.json](/dist/docs/3.0.0-rc.2/brand-protocol/brand-json)) |
...
See [Accounts Protocol overview](/dist/docs/3.0.0-rc.2/accounts/overview) for the full commercial model and [sync_accounts](/dist/docs/3.0.0-rc.2/accounts/tasks/sync_accounts) for the task reference.

## What sellers declare

Sellers configure the `account` section of [`get_adcp_capabilities`](/dist/docs/3.0.0-rc.2/protocol/get_adcp_capabilities#account):
...
Sellers can also declare `account_financials: true` to expose account-level financial data (spend, credit, invoices) via [`get_account_financials`](/dist/docs/3.0.0-rc.2/accounts/tasks/get_account_financials). This only applies to operator-billed accounts.
...
See [sync_accounts task reference](/dist/docs/3.0.0-rc.2/accounts/tasks/sync_accounts) for the full request/response schema.
