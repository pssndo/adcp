...
- Compliance coverage from the [policy registry](/dist/docs/3.0.0-rc.2/governance/policy-registry) -- community-maintained, no per-brand configuration required
- Seller-side verification via governance checks
- Full audit trail via `get_plan_audit_logs`

Set [`authority_level: "agent_limited"`](/dist/docs/3.0.0-rc.2/governance/campaign/specification#budget-authority-levels) with a `reallocation_threshold` to define guardrails. The governance agent handles the rest.
