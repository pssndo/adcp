...
Reporting in AdCP aligns with the [Targeting](/dist/docs/3.0.0-rc.2/media-buy/advanced-topics/targeting) system used for campaign setup, enabling consistent analysis across the campaign lifecycle. This unified approach means you can report on exactly what you targeted.

Performance data feeds into AdCP's [Accountability & Trust Framework](/dist/docs/3.0.0-rc.2/media-buy/index.mdx#accountability--trust-framework), enabling publishers to build reputation through consistent delivery and helping buyers make data-driven allocation decisions.

## Key Optimization Tasks

### Delivery Reporting
Use [`get_media_buy_delivery`](/dist/docs/3.0.0-rc.2/media-buy/task-reference/get_media_buy_delivery) to retrieve comprehensive performance data including impressions, spend, clicks, and conversions across all campaign packages.
...
### Campaign Updates
Use [`update_media_buy`](/dist/docs/3.0.0-rc.2/media-buy/task-reference/update_media_buy) to modify campaign settings, budgets, and configurations based on performance insights.
...
- See [Security](/dist/docs/3.0.0-rc.2/building/implementation/security) for implementation details
...
2. **Format level**: `reported_metrics` on a creative format declares what the format can produce (see [Reported Metrics](/dist/docs/3.0.0-rc.2/creative/formats#reported-metrics))
...
Reporting webhooks use the same payload structure as [`get_media_buy_delivery`](/dist/docs/3.0.0-rc.2/media-buy/task-reference/get_media_buy_delivery) with additional metadata:
...
Webhook delivery status is tracked through **AdCP's global task management system** (see [Task Lifecycle](/dist/docs/3.0.0-rc.2/building/implementation/task-lifecycle)).
...
See [Webhooks](/dist/docs/3.0.0-rc.2/building/implementation/webhooks) for detailed implementation guidance.
...
See [Conversion Tracking](/dist/docs/3.0.0-rc.2/media-buy/conversion-tracking/) for the complete setup flow, and [Optimization Goals](/dist/docs/3.0.0-rc.2/media-buy/conversion-tracking/#optimization-goals) for the `optimization_goals` array reference.

### Budget optimization
- **Reallocation** between high and low performing packages via [`update_media_buy`](/dist/docs/3.0.0-rc.2/media-buy/task-reference/update_media_buy)
...
- **Performance analysis** by creative asset via [`get_media_buy_delivery`](/dist/docs/3.0.0-rc.2/media-buy/task-reference/get_media_buy_delivery) with by-creative breakdowns
- **A/B testing** — assign multiple creatives with weights via `creative_assignments`
- **Refresh strategies** — swap creatives via [`sync_creatives`](/dist/docs/3.0.0-rc.2/creative/task-reference/sync_creatives) to prevent fatigue
...
The performance feedback system enables AI-driven optimization by feeding back business outcomes to publishers. See [`provide_performance_feedback`](/dist/docs/3.0.0-rc.2/media-buy/task-reference/provide_performance_feedback) for detailed API documentation.
...
Buyers can voluntarily share performance outcomes using the [`provide_performance_feedback`](/dist/docs/3.0.0-rc.2/media-buy/task-reference/provide_performance_feedback) task:
...
Reporting aligns with AdCP's [Targeting](/dist/docs/3.0.0-rc.2/media-buy/advanced-topics/targeting) approach, enabling:
...
- **[`get_media_buy_delivery`](/dist/docs/3.0.0-rc.2/media-buy/task-reference/get_media_buy_delivery)** - Retrieve delivery reports
- **[`update_media_buy`](/dist/docs/3.0.0-rc.2/media-buy/task-reference/update_media_buy)** - Modify campaigns based on performance
- **[Media Buy Lifecycle](/dist/docs/3.0.0-rc.2/media-buy/media-buys)** - Complete campaign management workflow
- **[Targeting](/dist/docs/3.0.0-rc.2/media-buy/advanced-topics/targeting)** - Brief-based targeting and overlays