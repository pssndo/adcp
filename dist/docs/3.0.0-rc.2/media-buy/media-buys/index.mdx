...
### 1. Creation Phase
Transform discovered products into active advertising campaigns using [`create_media_buy`](/dist/docs/3.0.0-rc.2/media-buy/task-reference/create_media_buy):
...
Once created, the media buy requires creative assets via [`sync_creatives`](/dist/docs/3.0.0-rc.2/creative/task-reference/sync_creatives):
...
- **Delivery Monitoring**: Track pacing and performance metrics with [`get_media_buy_delivery`](/dist/docs/3.0.0-rc.2/media-buy/task-reference/get_media_buy_delivery)
...
For complete details on optimization strategies, performance monitoring, standard metrics, and best practices, see **[Optimization & Reporting](/dist/docs/3.0.0-rc.2/media-buy/media-buys/optimization-reporting)**.
...
A display campaign running March 1-31, broken into weekly $2,000 flights with a dark period for lift measurement (abbreviated — see [`create_media_buy`](/dist/docs/3.0.0-rc.2/media-buy/task-reference/create_media_buy) for the full request shape):
...
See [Media Products - Placements](/dist/docs/3.0.0-rc.2/media-buy/product-discovery/media-products.mdx#placements) for complete placement documentation.
...
See [Media Products - Property Targeting](/dist/docs/3.0.0-rc.2/media-buy/product-discovery/media-products#property-targeting) for more on how products declare targeting flexibility.
...
- **[`create_media_buy`](/dist/docs/3.0.0-rc.2/media-buy/task-reference/create_media_buy)**: Instant to days
...
- **[`update_media_buy`](/dist/docs/3.0.0-rc.2/media-buy/task-reference/update_media_buy)**: Instant to days
...
- **[`get_media_buy_delivery`](/dist/docs/3.0.0-rc.2/media-buy/task-reference/get_media_buy_delivery)**: ~60 seconds (data aggregation)
...
For comprehensive error handling guidance including pending vs error states, response patterns, and recovery strategies, see [Error Handling](/dist/docs/3.0.0-rc.2/building/implementation/error-handling).

Media buy specific error codes are documented in each task specification and the [Error Handling Reference](/dist/docs/3.0.0-rc.2/building/implementation/error-handling).
...
1. Use [`get_products`](/dist/docs/3.0.0-rc.2/media-buy/task-reference/get_products) to find inventory
2. Select products that align with campaign objectives
3. Configure packages with appropriate targeting and formats
4. Create media buy with [`create_media_buy`](/dist/docs/3.0.0-rc.2/media-buy/task-reference/create_media_buy)

### Creative Integration
Coordinate with creative management:
1. Understand format requirements from selected products
2. Prepare assets using [Creative Management](/dist/docs/3.0.0-rc.2/media-buy/creatives/)
...
1. **Track delivery** with [`get_media_buy_delivery`](/dist/docs/3.0.0-rc.2/media-buy/task-reference/get_media_buy_delivery)
...
3. **Update campaigns** with [`update_media_buy`](/dist/docs/3.0.0-rc.2/media-buy/task-reference/update_media_buy)
...
- **[Product Discovery](/dist/docs/3.0.0-rc.2/media-buy/product-discovery/)** - Finding inventory for media buys
...
- **[Orchestrator Design Guide](/dist/docs/3.0.0-rc.2/building/implementation/orchestrator-design)** - Implementation best practices
