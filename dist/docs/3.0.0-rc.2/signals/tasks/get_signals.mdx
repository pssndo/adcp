...
| `account` | [AccountRef](/dist/docs/3.0.0-rc.2/building/integration/accounts-and-agents#account-references) | No | Account for this request. When provided, the signals agent returns per-account pricing options if configured. |
...
```json
{
  "$schema": "/schemas/3.0.0-rc.2/signals/get-signals-response.json",
...
```json
{
  "$schema": "/schemas/3.0.0-rc.2/signals/get-signals-request.json",
