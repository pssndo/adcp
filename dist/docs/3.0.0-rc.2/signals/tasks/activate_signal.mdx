...
| `account` | [AccountRef](/dist/docs/3.0.0-rc.2/building/integration/accounts-and-agents#account-references) | No | Account for this activation. Associates with a commercial relationship established via `sync_accounts`. |
| `pricing_option_id` | string | Yes (if signal has pricing options) | The pricing option selected from `pricing_options` in the [`get_signals`](/dist/docs/3.0.0-rc.2/signals/tasks/get_signals) response. Records the buyer's pricing commitment at activation time. Pass this same value in subsequent `report_usage` calls. |
...
```json
{
  "$schema": "/schemas/3.0.0-rc.2/signals/activate-signal-response.json",
...
See **[Webhooks](/dist/docs/3.0.0-rc.2/building/implementation/webhooks)** for complete details on webhook configuration and reliability.
