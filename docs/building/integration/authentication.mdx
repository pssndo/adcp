...
Use the public test agent to validate your authentication setup. Set your AAO API key as the `ADCP_AUTH_TOKEN` environment variable, then configure your client:
...
```json
{
  ...
  "auth": {
    "type": "bearer",
    "token": "sk_your_aao_api_key_here"
  }
}
```

Obtain an AAO API key from your account settings at [aao.adcontextprotocol.org](https://aao.adcontextprotocol.org). Each AAO organization has its own API key; use the key for the organization you want to bill.
