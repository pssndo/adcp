...
**Transport-specific task tracking:**
- **MCP**: Use [MCP Tasks](/docs/building/integration/mcp-guide#async-operations-via-mcp-tasks) — poll via `tasks/get` with `taskId`, retrieve result via `tasks/result`
- **A2A**: Subscribe to SSE stream for real-time updates
- **REST**: Poll with `task_id`, or use [push notifications](/docs/building/implementation/webhooks)
